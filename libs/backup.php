...
if (secure("status","SESSION")!="Administrator")
{
    ...
    $mysqlUserName= $BDD_user;
    $mysqlPassword= $BDD_password;
    $mysqlHostName= $BDD_host;
    $DbName= $BDD_base;
    $backup_name= 'backup'.date('Ymd-His').".sql";
    $tables= array("users","gatc_baseline","document_version","document_reference","document_language");
    Export_Database($mysqlHostName,$mysqlUserName,$mysqlPassword,$DbName,$tables, $backup_name );
	//$tables= array("document","association_table");
	$tables=array("document");
	Export_Database($mysqlHostName,$mysqlUserName,$mysqlPassword,$DbName,$tables, $backup_name );
	$tables=array("association_table");
	Export_Database($mysqlHostName,$mysqlUserName,$mysqlPassword,$DbName,$tables, $backup_name );
    function Export_Database($host,$user,$pass,$name,  $tables, $backup_name )
    {
		...
		$file=fopen("../saves/" .$backup_name, 'a');
		fwrite($file, $content);
		fclose($file);
